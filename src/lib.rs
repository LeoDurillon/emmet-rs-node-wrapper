...
use emmet_rs::{
  checker::{input_correctly_close, input_correctly_structured},
  statement::Statement,
};
...
mod test_expand {
  ...
  fn should_expand() {
    ...
    assert_eq!(
      Some("<div sx={{test:\"test\"}}></div>".to_string()),
      expand("div:sx={{test:\"test\"}}".to_string())
    );
    assert_eq!(
      Some("<div sx={{display:\"flex\",flexDirection:\"column\"}}></div>".to_string()),
      expand("div:sx={{display:\"flex\",flexDirection:\"column\"}}".to_string())
    );
  }
  ...
  fn should_ignore_before() {
    assert_eq!(
      Some("return <div></div>".to_string()),
      expand("return div".to_string())
    );
    assert_eq!(
      Some("return <div> test</div>".to_string()),
      expand("return div< test".to_string())
    );
  }
}
